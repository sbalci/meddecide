...
>
> **`treeadvanced()` — Advanced Clinical Decision Trees — is still in development and is
> not yet in the released meddecide module.** It carries a development menu group, so it
> does not appear in the jamovi menu in a released build and the R wrapper is not exported.
>
> This vignette moved here from the jjstatsplot collection, where it was written against
> the earlier name `advancedtree`. **It also documents more options than the analysis
> currently implements.** Of the arguments used below, these are not (yet) present in
> `jamovi/treeadvanced.a.yaml`:
>
> `algorithm`, `max_depth`, `n_estimators`, `importance_method`, `learning_rate`,
> `handle_imbalance`, `imbalance_method`, `cost_sensitive_thresholds`, `fn_fp_ratio`,
> `interpretability`
>
> Treat those sections as design intent rather than a usable interface. The options that
> do exist today include `vars`, `facs`, `target`, `targetLevel`, `validation`, `cv_folds`,
> `cv_repeats`, `bootstrap_samples`, `stratified_sampling`, `test_split` and
...
The `treeadvanced` function provides comprehensive clinical reporting capabilities:
