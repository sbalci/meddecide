...
analysis, enabling healthcare professionals to make evidence-based
clinical decisions through systematic evaluation of diagnostic tests,
treatment options, and health economic outcomes. This module combines
traditional decision tree analysis with advanced Markov modeling for
...
- **ROC Analysis**: Receiver operating characteristic curves and AUC
  calculation
...
- **Simple Decision Trees**: Binary choice scenarios with immediate
  outcomes
- **Complex Decision Trees**: Multi-branch decisions with sequential
  outcomes
- **Cost-Effectiveness Trees**: Economic evaluation integrated with
  clinical outcomes
...
The meddecide module is installed separately and provides specialized
tools for medical decision analysis.

#### In jamovi:

1.  Install the meddecide module from the jamovi Library
2.  Open the **meddecide** menu from the Modules toolbar
3.  Choose from: Agreement, Decision Panel, ROC, Screening Calculator,
    or Power Analysis
...
meddecide includes comprehensive datasets for decision analysis
training:
...
- For rule‑out (don’t miss disease): choose a threshold with very high
  Sensitivity (e.g., \>=0.95), accepting lower Specificity.
- For rule‑in (avoid overtreatment): choose a threshold with very high
  Specificity (e.g., \>=0.95), accepting lower Sensitivity.
- Consider prevalence and clinical consequences when selecting
  cutpoints; Youden’s J maximizes overall discrimination but may not
  reflect your clinical goal.
...
1.  **Problem Definition**: Clearly define the decision context and
    alternatives
...
2.  **Sample Size**: Adequate power for clinically meaningful
    differences
3.  **Cutpoint Selection**: Balance sensitivity and specificity based on
    clinical needs
...
1.  **Perspective**: Clearly state economic perspective (payer,
    societal)
2.  **Time Horizon**: Appropriate to capture all relevant costs and
    benefits
3.  **Discounting**: Apply standard discount rates for future
    costs/benefits
...
The meddecide module provides comprehensive tools for evidence-based
...
an essential tool for contemporary healthcare research and practice.
...
*This introduction provides the foundation for using meddecide in
