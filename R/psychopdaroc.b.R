  ...
  R6::R6Class(
    ...
    private = list(
      ...
      .processVariable = function(var, data, subGroup = NULL, params) {
        ...
        # Compute smoothed ROC curve for plotting
        if (!is.null(params$rocSmoothingMethod) && params$rocSmoothingMethod != "none") {
          neg_class <- setdiff(unique(classVar), params$positiveClass)
          if (length(neg_class) == 0) neg_class <- setdiff(levels(factor(classVar)), params$positiveClass)
          roc_obj <- pROC::roc(
            response = classVar,
            predictor = dependentVar,
            levels = c(neg_class[1], params$positiveClass),
            direction = ifelse(params$direction == ">=", ">", "<"),
            quiet = TRUE
          )
          roc_obj <- pROC::smooth.roc(roc_obj, method = params$rocSmoothingMethod)
          private$.smoothROCList[[var]] <- data.frame(
            threshold = roc_obj$thresholds,
            sensitivity = roc_obj$sensitivities,
            specificity = roc_obj$specificities
          )
          results$AUC <- as.numeric(pROC::auc(roc_obj))
        } else {
          private$.smoothROCList[[var]] <- NULL
        }
      ...
      .setupAnalysisParameters = function() {
        ...
        return(list(
          ...
          rocSmoothingMethod = self$options$rocSmoothingMethod
        ))
      },
      ...
      .preparePlotData = function(var, results) {
        ...
        if (!is.null(private$.smoothROCList[[var]])) {
          smooth <- private$.smoothROCList[[var]]
          j_max_idx <- which.min(abs(smooth$threshold - private$.optimalCriteriaList[[var]]$threshold))
          youden_val <- metrics$youden[which.min(abs(confusionMatrix$x.sorted - private$.optimalCriteriaList[[var]]$threshold))]
          data.frame(
            var = rep(var, nrow(smooth)),
            cutpoint = smooth$threshold,
            sensitivity = smooth$sensitivity,
            specificity = smooth$specificity,
            ppv = NA,
            npv = NA,
            AUC = rep(results$AUC, nrow(smooth)),
            youden = youden_val,
            j_max_idx = j_max_idx,
            stringsAsFactors = FALSE
          )
